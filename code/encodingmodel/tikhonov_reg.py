...
def _validate_ls(ls, X):
    ...
    try:
        ...
        assert ls[0].dtype is torch.float or ls.dtype is torch.double
        assert len(ls[0].shape) == 1
        assert len(ls[0]) == X.shape[1]
    except AssertionError:
        raise AttributeError(
            "invalid ls: should be a list of 1-dimensional torch float/double tensor and matches the number of columns "
            "in X"
        )
...
class Tikhonov:
    ...
    """Tikhonov regression model for multiple outputs and regularization strengths. A separate
    model is fit for each (output, regularization) pair."""

    def __init__(self, ls, scale_X=True, scale_thresh=1e-8):
        """
        Arguments:
                       ls: a list of 1-dimensional torch tensor of regularization values.
