...
class AE(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.decoder_hidden_layer = nn.Linear(
            kwargs["latent_dim"], out_features=64
        )
...
if __name__ == "__main__":
    ...
    train_data = NSDBrainOnlyDataset(args.output_dir, args.subj, train_idx, args.roi, args.roi_num)
    test_data = NSDBrainOnlyDataset(args.output_dir, args.subj, test_idx, args.roi, args.roi_num)
    ...
    for d in latent_dims:
        ...
        np.save("%s/autoencode/train_loss_dim%d_%s_%s.npy" % (args.output_dir, d, args.roi, roi_label), train_losses)
        np.save("%s/autoencode/test_loss_dim%d_%s_%s.npy" % (args.output_dir, d, args.roi, roi_label), test_losses)