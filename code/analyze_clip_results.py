...
if __name__ == "__main__":
    ...
    if args.plot_voxel_wise_performance:
        corr_i = load_model_performance(
            "convnet_res50_person_subset", None, args.output_dir, subj=args.subj
        )
        # corr_t = load_model_performance("clip_text", None, args.output_dir, subj=args.subj)
        corr_j = load_model_performance(
            "clip_person_subset", None, args.output_dir, subj=args.subj
        )

        # roi_cmap = np.zeros(corr_i.shape)
        colors = np.load(
            "output/voxels_masks/subj%01d/roi_1d_mask_subj%02d_%s.npy"
