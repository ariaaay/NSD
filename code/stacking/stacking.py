...
def stacking_CV_fmri(data, features, method = 'cross_val_ridge',n_folds = 4, with_subs = True, with_subc = False, with_concat = True):
    ...
    for ind_num in range(n_folds):
        ...
        for FEATURE in range(n_features):
            preds_train[FEATURE], error, preds_test[FEATURE,test_ind], r2s_train_folds[ind_num,FEATURE,:], var_train_folds[ind_num,FEATURE,:] = feat_ridge_CV(train_features[FEATURE],train_data,test_features[FEATURE],method=method)
        ...
        if with_subc:
            for ifeature in range(n_feat-1):
                ...
                _, _, concat_pred_sub[ifeature,test_ind,:], _, _ = feat_ridge_CV(train_feature_c,train_data,test_feature_c,method=method)
        ...
        for i in range(0,n_voxels):
            ...
            z = np.array([preds_test[feature_j,test_ind,i] for feature_j in range(n_feat)])
            # if i==0:
            ...
            if with_subs:
                for ifeature in range(n_feat):
                    ...
                    stacked_pred_sub[ifeature,test_ind,i] = np.dot(S_feature,z[ind_feature==1,:])
