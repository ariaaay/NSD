...
dirct = dict(
    collab="/content/gdrive/My Drive/",
    Darwin="/Users/lrg1213/",
    Linux="/share/volume0/lwehbe/MEG18/",
)[location]
...
def make_sub_figures(
    ...
    try:
        acc_time_sensor = np.load(
            prefix_results + "_time_sensor{}.npy".format(result_suffix)
        )
        acc_time_sensor_std = np.load(
            prefix_results + "_std_time_sensor{}.npy".format(result_suffix)
        )
        ...
        for row in ax:
            for col in row:
                if idx < nF:
                    ...
                    col.plot(
                        time, acc_time[-1] - s, "-", color=colors[idx], linewidth=2
                    )
                    col.plot(
                        time, (max_sig < min_sig_all) * 0.2, ".k"
                    )  # color=colors[idx])
                    col.title.set_text("\n" + titles[idx] + "\n")
    ...
    if make_spatial:
        ...
        for II in range(nF):
            ...
            plt.savefig(
                save_directory + prefix + "_sensor_time_{}.pdf".format(II),
                bbox_inches="tight",
            )
...
def smooth_run_not_masked(data, smooth_factor):
    ...
    for i, d in enumerate(data):
        smoothed_data[i] = gaussian_filter(
            data[i],
            sigma=smooth_factor,
            order=0,
            output=None,
