        ...
        if args.combined_roi:
            ...
            for roi in args.single_roi:
                ...
                for i, m in enumerate(roi_masks):
                    try:
                        rdm = np.load("%s/rdms/subj%02d_%s.npy" % (args.output_dir, args.subj, roi_labels[i]))
                    except FileNotFoundError:
                        brain_data = br_data[:, m]
                        rdm = np.corrcoef(brain_data)
                        np.save("%s/rdms/subj%02d_%s.npy" % (args.output_dir, args.subj, roi_labels[i]), rdm)
                    if args.biclustering:
                        ...
                        np.save("%s/rdms/subj%02d_%s_biclustering_indexes.npy" % (args.output_dir, args.subj, roi_labels[i]), bc_idx)
    ...
    if args.spectral_clustering:
        for roi in args.single_roi:
            ...
            for i, m in enumerate(roi_masks):
                ...
                np.save("%s/rdms/cluster_%d_labels_subj%02d_%s.npy" % (args.output_dir, args.subj, roi_labels[i]), clustering.labels_)

    if args.ID:
        sample_size = 3000
        for roi in args.single_roi:
            try:
                with open("../Cats/outputs/brain_ID_dict_%s_s%d.json" % (roi, sample_size), "r") as f: 
                    ID_dict = json.load(f)
            except FileNotFoundError:
                br_data, _ = load_brain_data(args)
                subsample_idx = np.random.choice(np.arange(br_data.shape[0]), size=sample_size, replace=False)
                ID_dict = {}
                roi_masks, roi_labels = extract_single_roi(roi)
                for i, m in enumerate(roi_masks):
                    brain_data = br_data[:, m]
                ...
                with open("../Cats/outputs/brain_ID_dict_%s_s%d.json" % (roi, sample_size), "w") as f: 
                    json.dump(ID_dict, f)

            plt.figure()
            plt.bar(np.arange(len(ID_dict.keys())), np.array(list(ID_dict.values()))[:,0], yerr = np.array(list(ID_dict.values()))[:,1])
            plt.xlabel("ROIs")
            plt.ylabel("Intrinsic Dimensions")
            plt.xticks(ticks=np.arange(len(ID_dict.keys())), labels=list(ID_dict.keys()))
