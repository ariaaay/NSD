...
overlap = pd.DataFrame({"cocoId": NSD_subset["cocoId"], "nsdId": NSD_subset["nsdId"]})

bold_list = pickle.load(
    open(
        "/home/yuanw3/ObjectEmbeddingSpace/outputs/convnet_features/convnet_image_orders_fc6.p",
        "rb",
    )
)
...
bold_overlap_list = [
    i
    for i, name in enumerate(bold_name_list)
    if "COCO" in name
    and sum(overlap["cocoId"].isin([name.split("_")[-1].split(".")[0][-6:]])) > 0
]
