...
def plot_max_diff_images(rsm1, rsm2, cocoId_subj, model_name_for_fig, print_caption=False, print_distance=False):
...
def sample_level_semantic_analysis(
    ...
    # bert = np.load("%s/subj%d/%s.npy" % (args.feature_dir, subj, "bert_layer_13"))
    ...
    print(threshold)
    print("number of pair of images selected: " + str(num_pairs))
    ...
    # compare brain image
    print("Computing brain diff...")
    bdata = np.load(
        "%s/output/cortical_voxels/averaged_cortical_responses_zscored_by_run_subj%02d.npy"
        % (args.output_root, subj)
    )
    b_diff = bdata[trial_id_pair[:,0], :] - bdata[trial_id_pair[:,1], :]
    b_diff_mean = np.mean(b_diff, axis=0)
    print(b_diff_mean.shape)
    np.save("%s/output/rdm_based_analysis/voxel_diff_%s_vs_%s_%.1f_%.1f_%s.npy" % (args.output_root, model1, model2, threshold[0], threshold[1], threshold[2]), b_diff_mean)
    ...
    for i, pair in enumerate(trial_id_pair[:30]):
        for j in range(2):
            ...
            plt.axis("off")
    plt.tight_layout()
    plt.savefig("figures/CLIP/RDM_based_analysis/%s_vs_%s_%.1f_%.1f_%s.png" % (model1, model2, threshold[0], threshold[1], threshold[2]))
...
if args.extract_keywords_for_roi:
    with open("%s/output/clip/roi_maximization/1000eng.txt" % args.output_root) as f:
        out = f.readlines()
    common_words = ["photo of " + w[:-1] for w in out]
    try:
        activations = np.load(
            "%s/output/clip/roi_maximization/1000eng_activation.npy" % args.output_root
        )
    except FileNotFoundError:
