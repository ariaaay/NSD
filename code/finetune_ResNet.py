...
def prepare_dataloader(num_workers=4, train_batch_size=200, val_batch_size=256):
    ...
    train_set = datasets.CocoCaptions(root = '/lab_data/tarrlab/common/datasets/COCO/train2017/',
                        annFile = '/lab_data/tarrlab/common/datasets/coco_annotations/captions_train2017.json',
                        transform=data_transforms["train"],
                        target_transform=np.random.choice)
    val_set = datasets.CocoCaptions(root = '/lab_data/tarrlab/common/datasets/COCO/val2017/',
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, captions in dataloaders[phase]:
                ...
                # print(type(captions))
                encoded = tokenizer(list(captions), padding=True, truncation=True, return_tensors="pt").to(device)
                # tokens = tokenizer.encode(captions, add_special_tokens=False)
                with torch.no_grad():
                    text_out = lmodel(encoded["input_ids"], token_type_ids=encoded["token_type_ids"])
            ...
            if phase == 'val':
                epoch_clip_acc = clip_running_corrects.double() / len(dataloaders[phase].dataset)

                print('CLIP Acc: {:.4f}'.format(epoch_clip_acc))
                writer.add_scalar('CLIP Accuracy/{}'.format(phase), epoch_clip_acc, epoch)

                if epoch_acc > best_acc:
                    best_acc = epoch_acc
                    best_model_wts = copy.deepcopy(model.state_dict())
                    torch.save(best_model_wts, "/user_data/yuanw3/project_outputs/NSD/output/finetune/resnet_finetune.p")
...
if __name__ == "__main__":
    ...
    bert = torch.hub.load('huggingface/pytorch-transformers', 'model', 'bert-base-cased')    # Download model and configuration from S3 and cache.
    bert = bert.to(device)
